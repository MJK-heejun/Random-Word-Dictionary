...
export class WordService {
    ...
    apiKey: string = `d7e00125cd33554cd1c1a509b7f0091f3558955bb416ee24b`;    
    ...
    public GetRandomWord(wordType: string) {
        let randomApi = `${this.baseUrl}words.json/randomWord?hasDictionaryDef=true&hasDictionaryDef=false&minCorpusCount=0&maxCorpusCount=-1&minDictionaryCount=1&maxDictionaryCount=-1&minLength=5&maxLength=-1&api_key=${this.apiKey}`;
    ...
    public GetDefinition(word: string) {        
        let url = `${this.baseUrl}word.json/${word}/definitions?limit=1&includeRelated=true&sourceDictionaries=all&useCanonical=true&includeTags=false&api_key=${this.apiKey}`;
        return this.http.get(url)
            .map((res: Response) => res.json()); //convert observable into json format
    }

    public GetAudioUrl(word: string) {
        let url = `${this.baseUrl}word.json/${word}/audio?useCanonical=true&limit=1&api_key=${this.apiKey}`;
        return this.http.get(url)
            .map((res: Response) => res.json()); //convert observable into json format
    }
